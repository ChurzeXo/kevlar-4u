...
| 编排 (兜底) | `orchestration` | 以上两种模式均不可用 | **V4 矩阵填空协议** — 单 prompt 含 6 个 XML 沙盒槽位 | **强化角色扮演** — 顺序执行人设，上下文重置门隔离 | 边界逃逸防御、结构化封装 |

> **v2.1 变更**: `direct_api` 模式已移除。原 L1 直接 API 调用路径已被 MCP Sampling 模式 + Host AI 的 `samplingFn` 注入覆盖。
...
| **超时与中断** | `src/execution/reviewSteps.ts` | `runSystemAuditors()` 使用 `Promise.race()` 实现超时控制，默认由 `multiAgent.timeoutMs` 配置 |
...
| **按模式预算** | `src/execution/aggregator.ts` | 三模式独立 session 预算：orchestration 8K、mcp_subagent 24K、mcp_sampling 32K tokens |
...
| 环境变量 | 默认值 | 说明 |
| --- | --- | --- |
| `KEVLAR_MODE` | `auto` | `auto`、`orchestration`、`mcp_sampling`、`mcp_subagent` |
