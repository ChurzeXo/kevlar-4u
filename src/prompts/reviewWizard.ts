...
【核心功能】
基于"攻防风险模拟"视角，对用户提交的文本进行深度攻击链推演与多维度社会语义风险评测。
...
export function buildOrchestrationAuditPrompt(
  ...
  const deterministicContextSection = preAuditContext
    ? [
        ...
        ``,
        `### Pipeline Step 0a：规则引擎输出 localFindings`,
...
export function buildOrchestrationFinalizerPrompt(
  ...
  return [
    ...
    `规则引擎的 findings 已合并进 network_culture_risk 维度。以下是合并后的完整维度数据：`,
    ``,
...
export function buildIsolatedSystemAuditorMessage(
  ...
  const localFindingsSection =
    localFindings.length > 0
      ? [
          `## 【规则引擎预警（已纳入 blackAtoms）】`,
          `以下发现来自规则引擎，已被 Turn 1 纳入解码基础；请在当前维度相关时纳入 findings：`,
          ``,
          JSON.stringify(localFindings, null, 2),
          ``,
        ].join("\n")
...
export function buildPreAuditFinalizerPrompt(
  ...
  return [
    ...
    ``,
    `### 原则三：规则平权仲裁`,
    `规则引擎（source: "local_rule_engine"）的发现仅作为候选输入。你拥有最高仲裁权，必须结合具体语境评估本地规则的报警是否成立，可以自由决定采纳、合并或过滤。`,
    ``,
    `## 【数据处理规则】`,
...
export function buildSubagentDispatchPrompt(params: {
  ...
  return [
    ...
    `## 结果聚合`,
    ``,
    `所有 subagent 完成后，请将 6 个维度的结果聚合为以下完整格式：`,
    ``,
    ...PRE_AUDIT_OUTPUT_FORMAT,
    ``,
    `注意：除了 dimensions、attackChainAnalysis 和 worstCaseNarrative 外，其余字段均为可选字段，如果不可用则设为 null 或省略。`,
    ...
    `## 降级策略（重要）`,
    ``,
    `如果你当前无法使用 Task/Subagent 工具（例如工具不可用、模型不支持并行调度），`,
    `请在当前 context 中串行逐一完成以上 ${auditors.length} 个维度的审计，`,
    `并调用 review_content_wizard 返回结果。返回的 JSON 必须严格遵循以下结构：`,
    `{`,
    `  "dimensions": [ ... ],`,
    `  "attackChainAnalysis": "...",`,
    `  "worstCaseNarrative": "...",`,
    `  "executionMode": "serialFallback"`,
    `}`,
  ].join("\n");
}
