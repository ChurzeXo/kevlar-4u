...
이 저장소는 **AGPL-3.0** 라이선스입니다. **Free**(오픈소스)와 **Pro**(구독 필요) 클라이언트 코드를 모두 포함합니다. Pro 기능은 유효한 라이선스 없이 작동하지 않습니다. 이 저장소에는 Pro 프롬프트 IP나 서버 측 코드가 없습니다.

### 기능 비교

| 영역 | Free (즉시 사용 가능) | Pro (활성화 필요) |
|---|---|---|
| **페르소나 시뮬레이션** | 완전한 RST 페르소나 생성, 자연어 파싱, 모든 실행 모드 | — |
| **시스템 사전 감사** | 6개 방어 차원 + 로컬 규칙 엔진 (`rules_free.json`) | 서버 동기화 전략 번들: 강화된 프롬프트, 실제 사례명, 최악 시나리오 |
| **규칙 세트** | `rules_free.json` (저장소 포함) | `rules_pro.json` 추가 (서버에서 가져옴) |
| **감사 보고서 상세** | 추상/일반적 설명 | 실제 브랜드/이벤트명, 상세 증폭 체인 |
| **전략 업데이트** | 릴리스에 번들된 정적 기본값 | 서버에서 동적 동기화 (`npx . --sync`) |
| **프롬프트 충실도** | 잠김/플레이스홀더 (프롬프트 IP 유출 방지) | SaaS의 전체 Pro 명령어 |
| **자격 증명 및 동기화** | — | AES-256-GCM 자격 증명 저장소, Ed25519 번들 서명 검증, 취소 확인 |
| **사용자 페르소나 저장** | `skills/*.json`을 통한 무제한 로컬 페르소나 | — |

> **Pro는 백엔드 서비스 `https://kevlar4u.xyz`에 의해 구동됩니다.** 클라이언트는 메타데이터(라이선스 ID, 버전, 세션 ID, 로케일)만 전송하며 사용자 콘텐츠, 감사 결과, API 키는 전송하지 않습니다.
...
### 1. 자유자재로 커스터마이징 가능한 리뷰어 (페르소나 커스터마이징) — Free
...
- **Stage 1 — 시스템 사전 감사** (Free: 로컬 규칙; Pro: 서버 강화): 6개의 전문 시스템 감사자가 컴플라이언스, 맥락 왜곡, 네트워크 문화 위험, 사실 오류, 사회적 위험, 언어 간 왜곡을 병렬 스캔하여 구조화된 발견 보고서를 생성합니다.
- **Stage 2 — RST 리뷰** (Free): RST 성격을 가진 사용자 생성 리뷰어가 **포커스 토픽**(사전 감사 발견을 필터링+번역)을 받고, 차원 점수 보고서가 아닌 실제 사용자 반응을 생성합니다.
...
```bash
npx . --activate --code <활성화 코드>    # 코드를 라이선스로 교환
npx . --sync                              # 서버에서 전략 번들 동기화
npx . --status                            # Free/Pro 상태 확인
npx . --doctor                            # 진단 실행
```

활성화 코드는 1회용이며 시간 제한이 있습니다(10~30분). 활성화 후 클라이언트는 AES-256-GCM으로 자격 증명을 안전하게 저장하고 `kevlar4u.xyz`에서 전략 번들을 동기화합니다. `--status`는 현재 티어를 표시하고 `--sync`는 서명 검증이 포함된 최신 Pro 번들을 다운로드합니다.
...
| MCP Subagent | `mcp_subagent` | Host AI의 Task/Subagent 도구를 활용한 병렬 감사 | Subagent 지원 클라이언트 (opencode, Claude Code 등) |
| Orchestration (호스트 보조 폴백) | `orchestration` | 호스트 AI가 완료를 지원, 낮은 격리의 폴백 | Sampling도 Subagent도 사용 불가능한 최후의 수단 |

> **v2.1**: `direct_api` 모드가 제거되었습니다. 직접 API 호출은 MCP Sampling + Host AI의 `samplingFn` 주입으로 통합되었습니다.

`auto` 모드 해결 순서:

1. `skills/kevlar-config.json`에 지정된 모드 사용 (설정된 경우)
2. 그렇지 않으면 `KEVLAR_MODE` 환경 변수 읽기
3. 그렇지 않으면 가용성에 따라 자동 선택: `mcp_sampling` → `mcp_subagent` → `orchestration`
...
| 변수 | 기본값 | 설명 |
| --- | --- | --- |
| `KEVLAR_MODE` | `auto` | `auto`, `orchestration`, `mcp_sampling` |
...
flowchart TD
  ...
  subgraph Pro["Pro (구독)"]
    ...
    Bundle["전략 번들<br/>(프롬프트, 규칙, 설정)"]
    Cred["AES-256-GCM 자격 증명 저장소"]
  end

  Sync --> Server --> Bundle --> Cred --> Execution
```

**Free** 기능(페르소나 생성, RST 리뷰, 로컬 규칙 엔진, 모든 실행 모드)은 완전히 오프라인에서 작동합니다. **Pro**는 서버 동기화 전략 번들(강화된 프롬프트, 실제 사례명, 추가 규칙 세트)을 추가합니다. `npx . --activate`로 활성화하고 `npx . --sync`로 동기화합니다.
...
│   ├── cli.ts                             # 인터랙티브 설치 CLI
│   ├── credentialCli.ts                   # Pro: 활성화, 라이선스, 동기화 CLI
...
│   │   ├── strategy.ts                    # Pro: 전략 계획 타입
│   │   ├── strategyBundle.ts              # Pro: 번들 서명 및 검증
│   │   ├── bundleStrategyProvider.ts      # Pro: 서버 기반 전략 제공자
│   │   ├── proRuntime.ts                  # Pro: 런타임 로더
│   │   ├── reviewSteps.ts                 # Pro: 단계 타입 시스템 및 실행
│   │   └── modes/
│   │       ├── orchestration.ts
│   │       ├── sampling.ts
│   │       └── subagent.ts
│   ├── credential/                        # Pro: 활성화, 라이선스, 동기화, 번들 캐시
│   │   ├── index.ts                       # AES-256-GCM 자격 증명 저장소
│   │   ├── activate.ts                    # 활성화 코드 → 라이선스
│   │   ├── activationClient.ts            # 전체 활성화 흐름
│   │   ├── bundleCache.ts                 # 번들 캐시 읽기/쓰기/상태
│   │   ├── syncClient.ts                  # 서버에서 전략 번들 동기화
│   │   └── store.ts                       # 디스크 기반 보안 자격 증명 저장소
│   ├── subscription/                      # Pro: SaaS-프롬프트 통합
│   │   ├── tier.ts                        # isPro() 결정
│   │   ├── promptTypes.ts                 # PromptSegments 타입 및 기본값
│   │   └── promptTemplates.ts             # Pro/Free 계층 프롬프트 텍스트
│   ├── tools/                             # MCP 도구
│   │   ├── index.ts                       # 도구 레지스트리
│   │   ├── listPersonasTool.ts
