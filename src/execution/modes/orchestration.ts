...
以下为各审查员的角色定义与待评审内容。请严格按顺序执行，**每个审查员之间设有明确的隔离边界，进入下一个审查员前必须重置思维上下文，丢弃上一个审查员的全部推理和结论**。

执行规则：
- 每个审查员只能基于自身角色定义 + 「待评审内容」独立判断，严禁在推理或结论中引用前一位审查员的输出
- 每位审查员必须完整输出 <cot_N>、<findings_N> 和结束标记后再进入下一位
- 汇总报告阶段可以引用 findings_N，但各审查员执行阶段绝对禁止跨角色引用
...
--- 隔离边界：上下文重置点。丢弃上一个审查员的全部推理和结论，仅基于【待评审内容】重新分析 ---

请按以下结构输出（不得省略任何标签，不得跨标签引用其他审查员的发现）：
