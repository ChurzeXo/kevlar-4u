...
Kevlar-4uのコアローカル機能は**AGPL-3.0**ライセンスの下でオープンソース化されています。

クラウドベースのリスクワードクラウドサービス、有料ルール同期、および高度な機能は**プロプライエタリな商用サービス**です。
...
- **Stage 1 — システムプレ監査**：5つの専門システム監査人がコンプライアンス、文脈歪み、ネットワーク文化リスク、事実誤り、社会的リスクを並列スキャンし、構造化された発見レポートを生成。
- **Stage 2 — RSTレビュー**：RST人格を持つユーザー作成レビュアーが**フォーカストピック**（プレ監査の発見をフィルタリング+翻訳）を受け取り、次元スコアリングレポートではなく、本物のユーザーリアクションを生成。
