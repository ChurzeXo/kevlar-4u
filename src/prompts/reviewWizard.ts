...
**功能**：基于"职业黑粉逆向解码"视角，对用户提交的文本进行攻击链推演与多维度社会语义风险评测（合规、语境脱嵌、网络文化、事实、社会风险），输出结构化 JSON。

**触发时机**：用户提交文本内容并要求评测风险、审稿、或类似表述时调用。

**核心流程**：
1. 执行系统初审，输出结构化 JSON 结果。
2. 展示初审结果，等待用户明确执行复审还是平台合规检查。

**严格规则**：
- 必须等待用户明确回复才能执行下一步，严禁自动推进
- 输入必须为纯文本（不支持图片、PDF、文档等文件）
- 禁止提供任何修改建议、优化方向、文案润色或重写意见`;
...
export function buildPreAuditFinalizerPrompt(systemAuditors: Persona[]): string {
  return [
    ...
    `能推演出完整攻击链的：原始表达 → 去语境化呈现 → 评论区反应 → 舆情走向`,
    `    → 无论风险类型，直接标记为 🔴`,
    ``,
    `### 原则二：最坏解读原则`,
    `对所有风险点进行「最坏解读」评估：`,
    `假设被恶意断章取义、带节奏后，最恶劣的解读会是什么？`,
    ...
    ``,
    `## 协同加权与脱嵌 delta 信号（必须综合考虑）`,
    ``,
    `输入数据中包含以下两个重要信号，你必须在风险等级校准时综合考虑：`,
    ``,
    `### 1. synergy（协同加权结果）`,
    `- 当多个维度同时触发风险时，组合效应会放大实际风险`,
    `- 如果 synergy.triggered 非空，说明存在跨维度组合风险`,
    `- synergy.overallMultiplier > 1.0 时，应考虑将相关维度的 🟡 升级为 🔴`,
    ``,
    `### 2. deltaRisks（脱嵌 delta 信号）`,
    `- bareOnly：仅在裸文（脱嵌后）中出现的风险，代表「脱语境放大」型风险`,
    `- fullOnly：仅在全文中出现的风险，代表需要完整上下文才能识别的风险`,
    `- stable：两轮共有的稳定风险`,
    `- 如果 bareOnly 非空，说明存在「脱嵌型风险放大」，应优先关注并可能升级等级`,
    ``,
    `## 你的任务`,
    `- 合并重复风险，保留最高风险等级`,
    ...
    ``,
    `summary 字段的写法要求：`,
    `- 如有 🔴 风险：直接描述攻击链和具体攻击路径（如「存在被断章取义的风险」「存在被主动裁切二次创作的风险」等）`,
    `- 如有 🟡 风险：描述风险触发条件`,
