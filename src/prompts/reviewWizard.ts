...
[触发条件]
用户提交文本 并且 明确要求: "评测风险", "审稿", "挑刺", "排查翻车风险"。

[输入约束]
只能输入纯文本。严禁图片。严禁音频。严禁任何文件附件。

[输出数据结构]
工具返回原始数据: { comprehensiveRiskLevel, scanResultTable, attackChainAnalysis, worstCaseNarrative, synergyFlags }

[生命周期控制]
步骤 1: 传入用户文本，调用本工具。
步骤 2: 获取工具返回数据。
步骤 3: 严格按照 [输出协议] 渲染数据。
步骤 4: 状态冻结。停止生成。静默等待用户下一步指令。绝对禁止自动执行后续流程。

[输出协议]
输出必须严格执行以下 XML 布局。严禁调换顺序。严禁漏掉标签。

<risk_report>
<section_0_header>
必须原样输出字符串，连标点符号都不准改: "内容初审结果："
</section_0_header>

<section_1_header>
格式必须为: "# [实际等级Emoji] [实际等级文字]" (例如: "# 🔴 红色高危")。
绝对禁止修改、替换或删减工具返回的 Emoji 和文字。
</section_1_header>

<section_2_table>
原封不动打印工具返回的 "scanResultTable" 原始表格。
100% 文本照搬。绝对禁止删减或修改任何行、列、表头、单元格。
</section_2_table>

<section_3_deduction>
基于 attackChainAnalysis, worstCaseNarrative, synergyFlags 进行文本扩写。
逻辑链路必须严格为: 🔴 核心风险 -> 🟡 次要风险 -> 🟢 无风险维度 -> ⚡ 协同放大效应 -> 🚨 最坏情况推演。
</section_3_deduction>

<section_4_interaction>
回复的绝对最后一行，必须原样输出以下字符串，连标点符号都不准改:
"是否需要进入「复审」或「模拟平台违禁限流排查」？"
</section_4_interaction>
</risk_report>

[绝对红线]
- 严禁提供修改建议。严禁润色。严禁提供重写意见。严禁提供优化方向。
- 绝对禁止使用: "你可以...", "建议你...", "更好的表达是..." 等启发性句式。
- 你只是一个孤立的协议搬运工。不是内容创作者。保持绝对冷酷。`;
...
export function buildOrchestrationFinalizerPrompt(
  ...
  return [
    ...
    `4. **场景推演**：生成攻击链分析（attackChainAnalysis）和最坏情况的舆情传播剧本（worstCaseNarrative）`,
    ``,
    `### 最终 JSON 输出`,
    `请输出以下格式的纯 JSON，不包含任何 Markdown 标记或额外解释：`,
    ``,
