...
### 1. 자유자재로 커스터마이징 가능한 가상 댓글러 (페르소나 커스터마이징)
...
- **정밀 매칭**: 가장 관련성 높은 가상 댓글러를 동적으로 필터링하고 스케줄링합니다.
...
| Wizard 도구 | 목적 | 주요 동작 |
| --- | --- | --- |
| `review_content_wizard` | 콘텐츠 리뷰 | 콘텐츠 제출 → 댓글러 선택 → 확인 → 다차원 피드백 |
...
`review_content_wizard`는 "콘텐츠 저장, 댓글러 선택, 실행 확인"을 하나의 안정적인 흐름으로 연결합니다.
...
| 모드 | 식별자 | 설명 | 적합한 환경 |
| --- | --- | --- | --- |
| MCP Sampling | `mcp_sampling` | 각 댓글러가 독립적인 샘플링 요청을 받음, 최대 격리 | Sampling을 지원하는 클라이언트, 진정한 다각도 리뷰를 원하는 경우 |
...
| `KEVLAR_MAX_CONCURRENT` | `3` | 최대 동시 댓글러 수 |
...
- **적응형 실행**: MCP Sampling이 가능하면 이를 필드 추출 및 댓글러 추천에 사용하고, 그렇지 않으면 휴리스틱 로직 또는 호스트 보조 오케스트레이션으로 폴백합니다.
