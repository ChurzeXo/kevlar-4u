...
| MCP Subagent | `mcp_subagent` | Host AIのTask/Subagentツールを利用した並列監査 | Subagent対応クライアント（opencode、Claude Code等） |
| Orchestration（ホスト補助フォールバック） | `orchestration` | ホストAIが補完、低分離のフォールバック | SamplingもSubagentも利用できない場合の最終手段 |

> **v2.1**: `direct_api` モードは削除されました。直接API呼び出しは MCP Sampling + Host AI の `samplingFn` 注入に統合されました。

`auto` モードの解決順序：

1. `skills/kevlar-config.json` で指定されたモードを使用（設定されている場合）
2. なければ `KEVLAR_MODE` 環境変数を読み取り
3. なければ利用可能性に応じて自動選択：`mcp_sampling` → `mcp_subagent` → `orchestration`
