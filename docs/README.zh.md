...
| 编排 (兜底) | `orchestration` | 既无 Sampling 又无 API Key | **V4 矩阵填空协议** — 单 prompt 含 6 个 XML 沙盒槽位 | **强化角色扮演** — 顺序执行人设，上下文重置门隔离 | 边界逃逸防御、结构化封装 |
...
| **按模式预算** | `src/execution/aggregator.ts` | 三模式独立 session 预算：orchestration 8K、mcp_sampling 32K、direct_api 40K tokens |
