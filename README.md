...
- **🚀 三大灵活执行模式（Flexible Execution Modes）**：支持 **宿主辅助兜底模式**（零额外成本、低隔离 fallback）、**MCP Sampling 并行采样**（深度高隔离）及 **Direct API 直连模式**（无 MCP 客户端环境），适应不同客户端能力与部署条件。
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **宿主辅助兜底模式** | `orchestration` | Server 将所有 Persona 指令聚合到**单次 Prompt** 中，交给宿主 AI 协助完成。属于低隔离 fallback，不等价于真正并行多智能体执行。 | 🪙 **零额外成本**<br>(Kevlar 本身不调用模型) | 否 *(宿主单次执行)* | 宿主模型的上下文与理解力 | 无 Sampling / API Key 时的最后兜底，适合低成本粗评。 |
...
🤖 AI: (自动调用 get_execution_modes 工具) 
       ...
       | 模式 | 状态 | 说明 |
       |------|------|------|
       | 宿主辅助兜底模式 (orchestration) | ✅ 可用 | 单次 Prompt 交给宿主 AI，零额外成本，低隔离 fallback |
...
*   **宿主辅助兜底模式**：会在生成的报告中标注 `执行模式：宿主辅助兜底模式`。
*   **采样 / 直连 API 模式**：会在生成的报告头部以醒目的元数据区域标出：
    ```markdown
    > 🛡️ **Kevlar 内容防弹评审报告**
    > **执行模式**：MCP 采样模式 (mcp_sampling)
    > **参与评论员**：急性子路人甲、键盘侠·九段杠精（共 2 位）
    > **部分失败**：无
    ```
