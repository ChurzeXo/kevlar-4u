...
export const updatePersonaDraftToolDefinition: Tool = {
	...
	description: `角色构建草稿暂存工具。在阶段式人设创建流程中，每当用户确认一个字段后立即调用本工具保存该字段。字段对应关系：年龄段 → ageRange，兴趣方向 → interests（字符串数组，≤3个标签），性格特质 → traits（字符串数组，≤4条「特质 → 行为」格式），常用平台 → platform。所有字段收集完毕并经用户最终确认后，调用 create_persona 完成创建。`,
	inputSchema: {
		type: "object",
		properties: {
			sessionId: {
