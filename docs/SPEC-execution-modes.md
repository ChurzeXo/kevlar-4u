...
| 模式 | 标识符 | 目标用户 |
|------|--------|----------|
| **宿主辅助兜底模式** | `orchestration` | 无 Sampling / API Key 时的低隔离 fallback |
...
用户选 auto
  ...
  → 检查: KEVLAR_MODE 环境变量是否设置？      是 → 使用环境变量指定模式
...
### 2.1 宿主辅助兜底模式（orchestration）

#### 原理
Kevlar Server 将所有 Persona 指令聚合到**单次 Prompt** 中，交给宿主 AI Client 协助完成。该模式零额外成本，但不是真正隔离的多智能体执行，只作为兜底 fallback。
...
const handlers: ExecutionHandler[] = [
  samplingHandler,        // priority: 10（最高优先级，首选）
  directApiHandler,       // priority: 20（次选）
  orchestrationHandler,   // priority: 30（最低优先级，兜底）
];
...
- Orchestration 模式：报告中标注 `执行模式：宿主辅助兜底模式`
...
| 模式 | 状态 | 说明 |
|------|------|------|
| 宿主辅助兜底模式 | ✅ 可用 | 单次 Prompt 交给宿主 AI，零额外成本，低隔离 fallback |
