...
向用户展示以下选项（用户可回复编号或文字）：
1. 已关注（信任阈值较高，但期望值也更高，容易因内容质量下滑而失望）
2. 未关注（信任阈值较低，更容易因细节问题流失注意力）

用户选择后，直接记录并写入临时记忆文档，然后进入最终确认。
...
用户在最终确认阶段提出修改内容时：
1. 识别用户要修改的字段：年龄段、兴趣方向、性格特质、常用平台或与作者的关系
2. 对应字段重新提炼或记录
3. 更新临时记忆文档中对应内容
4. 重新展示完整最终确认信息
...
当所有五个字段（年龄段、兴趣方向、性格特质、常用平台、与作者的关系）全部收集完毕，
